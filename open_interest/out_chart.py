...
chartExists=os.path.exists("out_charts")
if(not chartExists):
    os.makedirs("out_charts")
    
dataExists=os.path.exists("flags")
if(dataExists):
    company_list = os.listdir('flags')
else:
    exit('Data does not exists')

for company in company_list:
    try:
        data = pd.read_csv("flags/{}".format(company))
        n = len(data)
        ...
        plt.savefig('out_charts/VWAP_{}.png'.format(company))
        plt.clf()
